...
# its OS packages (e.g. openssl, gnutls).
...
# Uvicorn: single worker per container. With AXIOM_REDIS_URL, replicas share
# the response cache and rate limits; the LLM concurrency limit, the audit
# store and the SQLite corpus stay per process (README → Scaling out).

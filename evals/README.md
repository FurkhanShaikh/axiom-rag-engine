...
| 3b | `pipeline_retrieval_eval.py` | SciFact (dev split) | Production-shaped retrieval: a 10-document search pool per claim (gold + hardest distractors) through the **shipped** retriever → scorer → ranker. p@1, MRR, and evidence recall after the `max_ranked_chunks` trim; `--variant bm25_only` isolates the quality blend. Deterministic; gated per PR. |
